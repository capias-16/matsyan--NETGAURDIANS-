...
const projectName = "matsyan-bluewave"; 
...
const files = [
    {
        path: path.join(rootDir, 'public', 'index.html'),
        content: `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>PhantomNet SAR Detection Dashboard</title>
    <!-- Tailwind CSS CDN -->
...
<body>
    ...
    {
        path: path.join(rootDir, 'src', 'index.js'),
...
`
    ...
    {
        path: path.join(rootDir, 'src', 'App.js'),
        content: `import React from 'react';
...
`
    ...
    {
        path: path.join(rootDir, 'src', 'components', 'MainComponent.jsx'),
        content: `"use client";
...
`
  ...
  "browserslist": {
    ...
    {
        path: path.join(rootDir, '.gitignore'),
        content: `# Logs
logs
*.log
npm-debug.log*
...
// Create files
files.forEach(file => {
    fs.writeFileSync(file.path, file.content.replace(/\\`/g, '`'), 'utf8'); // Replace escaped backticks
    console.log(`Created file: ${file.path}`);
});

console.log(`\nProject structure for "${projectName}" created successfully!`);
console.log(`\nNext steps:`);
console.log(`1. Navigate into the project directory: cd ${rootDir}`);
